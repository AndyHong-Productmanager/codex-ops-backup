...
- CLI-only `higgsedit do PROJECT compose --params '{"dryRun":true,...}'`: non-mutating
  validation/compiled layout for a concrete unresolved layout question (replace `...`
  with the documented payload). Optional, not a pre-build ritual; no clip needs removal.
  `p.compose` options do not support `dryRun`; do not use it in scripts.
...
Native video cursors reject FFmpeg failures, incomplete frames and known premature EOF
instead of repeating stale pixels. Errors identify source, requested time and decoder
exit or signal. A successful source end can hold the last frame; unknown-length media
keeps that behavior on clean exit. This prevents frozen tails but does not diagnose why
the decoder failed. Lowering concurrency is not proof that a corrupt source was repaired.
