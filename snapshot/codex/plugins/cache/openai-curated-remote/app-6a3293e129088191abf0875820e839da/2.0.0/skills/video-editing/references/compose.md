...
Only for a concrete unresolved layout question, use the CLI-only
`higgsedit do PROJECT compose --params '{"dryRun":true,...}'` (replace `...` with the
documented compose payload). It is non-mutating: inspect the returned compiled layout;
it adds no clip and needs no cleanup. `p.compose` options do not support `dryRun`.
Do not pass it to the script API or insert a disposable probe clip. This is optional,
not a mandatory step before each build.
