...
name: ai-host-video
description: >
  Produce a complete channel episode carried by one consistent AI presenter,
  locked to an accepted image or video avatar, from a supplied script or topic.
  Includes host casting, spoken clips, supporting visuals, authored motion and
  a measured final edit. Use for a finished AI-hosted episode, not a standalone
  avatar clip, a faceless narrated video, a product UGC ad, script-only writing,
  adding a narrator to existing footage, or publishing an existing video.
metadata:
